...
col21.markdown("Here is the full results table.")
col21.dataframe(limit_data(filtered), use_container_width=True)
# Hits per Target/Treatment
colr22.markdown("Here you can see which Treatment/Target configuration provides the most hits for the chosen pvalue threshold.")
per = colr22.selectbox("Hit counts", ["TREATMENTS", "TARGET"])
nhits = filtered.groupby([per]).size().reset_index(name='COUNTS')
colr22.dataframe(nhits)
# Plot Pvalues
